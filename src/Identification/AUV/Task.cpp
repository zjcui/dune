...
namespace Identification
{
  namespace AUV
  {
    ...
    struct Task: public DUNE::Tasks::Periodic
    {
      ...
      initMatrices()
      {
        P_x(0,0) = m_args.Surge_cov1;
        P_x(1,1) = m_args.Surge_cov2;

        theta_x(0,0) = m_args.Surge_ldi;
        theta_x(1,0) = m_args.Surge_qdi;

        P_k(0,0) = m_args.Roll_cov1;
        P_k(1,1) = m_args.Roll_cov2;
        P_k(2,2) = m_args.Roll_cov3;
        P_k(3,3) = m_args.Roll_cov4;

        theta_k(0,0) = m_args.Roll_mcoef;
        theta_k(1,0) = m_args.Roll_ffcoef;
        theta_k(2,0) = m_args.Roll_ldi;
        theta_k(3,0) = m_args.Roll_qdi;
      }
